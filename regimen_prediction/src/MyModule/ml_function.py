...
def train_and_test(model_name, **kwargs):
    ...
    return accuracy, auc, f1,  shap_values
...
def test_model(test_data, model, scaler = None):
    ...
    tpred = model.predict_proba(x)

    ty = transform_label(y)

    accuracy = accuracy_score(y, pred)
    auc = roc_auc_score(ty, tpred, multi_class="ovr", average="weighted", labels=5)
...
def get_knn(model_name, train, valid, param, scale=True) : 
    ...
    for n in param['n_neighbors']:
        ...
        cur_score = roc_auc_score(valid_y, pred, average="weighted", multi_class="ovr", labels=5)
...
def tree_like(model_name,
    ...
    for i in range(len(param['max_depth'])):
        for k in range(len(param['min_samples_split'])):
            ...
            pred = model.predict(valid_x)
            tpred = model.predict_proba(valid_x)

            tvalid_y = transform_label(valid_y)
            cur_score = roc_auc_score(tvalid_y, tpred, average="weighted", multi_class="ovr", labels=5)
