...
def get_confidence_interval(test, label, pred,  seed) :
    n_bootstraps = 100
    ...
    for i in range(n_bootstraps):
        ...
        score = test(label[indices], pred[indices])
...
def test_model(model_list, type, epsilon, test, age, model_path, args) :
    test_x, test_y = test
    ...
    for idx,(model, scaler) in enumerate(model_list):
        ...
        auroc = get_confidence_interval(roc_auc_score, test_y, pred, args.random_seed)
        # auroc = roc_auc_score(test_y,pred)
        f1score = get_confidence_interval(f1_score, test_y, pred, args.random_seed)
        # f1score = f1_score(test_y, pred)
        accuracy = get_confidence_interval(accuracy_score, test_y, pred, args.random_seed)
        # accuracy = accuracy_score(test_y, pred)
        auprc = get_confidence_interval(average_precision_score, test_y, pred, args.random_seed)
...
def main() :
    ...
    rr_results = test_model(rr_models, 'trtr', 0, (test_x, test_y.reset_index(drop=True)), args.age, model_path, args)
    ...
    for idx, epsilon in enumerate(config['epsilon']) :
        ...
        for j, train_type in enumerate(['tstr', 'trts', 'tsts']) :
            result = test_model(strategy_list[j], train_type, epsilon, (test_x, test_y.reset_index(drop=True)), args.age, model_path, args)
